    <%@ page language="java" contentType="text/html; charset=UTF-8"
        ...
        <script src="${alfred }/layui/layui.js"></script>
        <script type="text/javascript">
        var tableIns;
        layui.extend({
        dtree:'examination/layui_ext/dist/dtree'
        ...
        ,url:'examination/menu/loadAllMenu.action' //数据接口
        ...
        alert(params);
        tableIns.reload({
        url:"examination/menu/loadAllMenu.action?"+params
        })
        });
        ...
        if(layEvent === 'del'){ //删除
        //先判断当前菜单有没有子节点
        $.post("examination/menu/checkMenuHasChildren.action?id="+data.id,function(obj){
        if(obj.code>=0){
                layer.msg("当前菜单有子节点，请先删除完子节点！");
        }else{
        layer.confirm('确定删除【'+data.title+'】这个菜单吗？', function(index){
        //向服务端发送删除指令
        $.post("examination/menu/deleteMenu.action",{id:data.id},function(res){
        layer.msg(res.msg);
        //刷新数据 表格
        tableIns.reload();
        //刷新左边的树
        ...
        url="examination/menu/addMenu.action";
        }
        });
        }
        //打开修改页面
        ...
        //移除打开的样式
       $("#menuSelectDiv").removeClass("layui-show");
        url="examination/menu/updateMenu.action";
        //反选下拉树
        var pid=data.pid;
        ...
        tableIns.reload({
        url:"examination/menu/loadAllMenu.action?id="+id
        })
        }
        </script>
        </body>
