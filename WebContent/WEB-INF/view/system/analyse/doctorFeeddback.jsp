...
function submitReply(_id) {
	...
	var parentId = -1;
	if (_id.indexOf("comment_content_reply") > -1) {
...
function feedbackDetail(data) {
	$.ajax({
		url : '/examination/comment/loadAllComment.action',
		type : "post",
		dataType : "json",
		async : false,//这得注意是同步
		data : {
...
// 	var dataArr = JSON.parse(data);
	...
	if (dataArr != null && dataArr.length > 0) {
		for (var i = 0; i < dataArr.length; i++) {
			...
			var comment_content_reply_id = "comment_content_reply" + "_" + o.id;
			html += '<ul style="border: 1px #009688 solid;margin-top: 10px;">'
				...
				+ '</div>'
				+ '<div style="width: 100%;margin-top: 10px;margin-left: 10px;">' + o.content
				+ '</div>'
				...
				+ '	<form id="replyForm">'
				+ '		<textarea class="comment-content" name="comment_content_reply" id="'+comment_content_reply_id+'" placeholder="用户反馈" maxlength="1000"></textarea>'
				+ '		<div class="opt-box">'
